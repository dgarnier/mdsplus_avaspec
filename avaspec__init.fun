public fun avaspec__init(as_is _nid, optional in _method)
{
  ...
  _trig_event = char(if_error(DevNodeRef(_nid, _AVASPEC_TRIG_EVENT), "");
  ...
  _status = avaspec->Init(val(_spec_no), val(_int_time), ref(_trig_event),ref(_triggers), val(_average), val(_dynamic), val(_max_spectra));
   return(_status != -1);
} 
